...
public class EB_AFIT : IPackingAlgorithm
{
    ...
    {
        // Store the items that cannot be placed in this container due to feature mismatch, e.g. refrigeration

        var itemsCannotBePacked=items.Where(a =>
           a.RequiredFeatures!=null && !a.RequiredFeatures.All(f => container.Features!=null && container.Features.Any(cf => cf == f))).ToList();

        items = items.Where(a => itemsCannotBePacked.All(c => c.ID != a.ID)).ToList();
        
        Initialize(container, items);
        ExecuteIterations(container,ct);
        Report(container,ct);
        ...
        for (var i = 1; i <= itemsToPackCount; i++)
        {
            ...
            if (!itemsToPack[i].IsPacked) result.UnpackedItems.Add(itemsToPack[i]);
        }
        
        foreach (var unpackable in itemsCannotBePacked)
            result.UnpackedItems.Add(unpackable);

        result.PackedItems = itemsPackedInOrder;


        if (result.UnpackedItems.Count == 0) result.IsCompletePack = true;
    ...
    private double totalContainerWeight;
    ...
    /// </summary>
    private void AnalyzeBox(double hmx, double hy, double hmy, double hz, double hmz, double dim1, double dim2,
        double dim3)
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (smallestZ.Pre == null)
                    {
                        ...
                        if (smallestZ.Post != null) smallestZ.Post.Pre = smallestZ;
                    }
                    ...
                    else
                    {
                        if (smallestZ.Pre.CumZ == smallestZ.Post.CumZ)
                        {
                            ...
                            if (smallestZ.Post.Post != null) smallestZ.Post.Post.Pre = smallestZ.Pre;
                        ...
                        else
                        {
                            ...
                            if (smallestZ.Pre.CumZ < smallestZ.Post.CumZ) smallestZ.Pre.CumX = smallestZ.CumX;
                        }
                    }
                }
            }
    ...
    /// </summary>
    private void ExecuteIterations(Container container,CancellationToken ct)
    {
        var strDebug = $"Executing iterations with container Id {container.ID}";
        AWSXRayRecorder.Instance.BeginSubsegment(strDebug);
        Console.WriteLine(strDebug);
        int itelayer;
        int layersIndex;
        var bestVolume = 0d;

        for (var containerOrientationVariant = 1;
             containerOrientationVariant <= 2 && !quit;
             containerOrientationVariant++)
        {
            ...
            for (layersIndex = 1; layersIndex <= layerListLen && !quit; layersIndex++)
            {
                ...
                for (x = 1; x <= itemsToPackCount; x++) itemsToPack[x].IsPacked = false;
    ...
    private void FindBox(double hmx, double hy, double hmy, double hz, double hmz)
    {
        ...
        for (y = 1; y <= itemsToPackCount; y = y + itemsToPack[y].Quantity)
        {
            ...
            // Only consider rotation along the horizontal axis
    ...
    /// </summary>
    private void FindLayer(double thickness,CancellationToken ct)
    {
        double exdim = 0;
        double dimdif;
        double dimen2 = 0;
        ...
        eval = 1000000;
           ...
           for (int i =0;i<itemsToPackCount;i++)
           {
               if (itemsToPack[i].IsPacked)
                   continue;
                    ...
                    for (y = 1; y <= 3; y++)
                    {
                        ...
                        {
                            case 1:
                                exdim = itemsToPack[i].Dim1;
                                dimen2 = itemsToPack[i].Dim2;
                                dimen3 = itemsToPack[i].Dim3;
                                break;

                            case 2:
                                exdim = itemsToPack[i].Dim2;
                                dimen2 = itemsToPack[i].Dim1;
                                dimen3 = itemsToPack[i].Dim3;
                                break;

                            case 3:
                                exdim = itemsToPack[i].Dim3;
                                dimen2 = itemsToPack[i].Dim1;
                                dimen3 = itemsToPack[i].Dim2;
                                break;
                        }
    ...
    private void FindSmallestZ()
    {
        ...
        {
            if (scrapmemb.Post.CumZ < smallestZ.CumZ) smallestZ = scrapmemb.Post;
    ...
    private void Initialize(Container container, List<Item> items)
    {
        ...
        itemsToPack.Add(new Item("0", 0, 0, 0, 0, null,null,0));
        ...
        foreach (var item in items)
        {
            ...
            {
                var newItem = new Item(item.ID, item.Dim1, item.Dim2, item.Dim3, item.Quantity, item.ItemTypeId,null,0);
        ...
        totalContainerWeight = container.MaxWeight;
        totalItemVolume = 0d;

        for (x = 1; x <= itemsToPackCount; x++) totalItemVolume = totalItemVolume + itemsToPack[x].Volume;
        ...
        for (y = 1; y <= 3; y++)
        {
            ...
            for (z = 1; z <= itemsToPackCount; z++)
                if (!(x == z))
                {
                    ...
                    if (Math.Abs(exdim - itemsToPack[z].Dim2) < dimdif) dimdif = Math.Abs(exdim - itemsToPack[z].Dim2);
                    if (Math.Abs(exdim - itemsToPack[z].Dim3) < dimdif) dimdif = Math.Abs(exdim - itemsToPack[z].Dim3);
                    layereval = layereval + dimdif;
                }
    ...
    /// </summary>
    private void Report(Container container,CancellationToken ct)
    {
        quit = false;
    ...
    private void VolumeCheck()
    {
        ...
        packedWeight = packedWeight + itemsToPack[cboxi].Weight;
        ...
        if (packedWeight >= totalContainerWeight)
        {
            packing = false;
            hundredPercentPacked = true;
        }
